...
("c.push({ k: 'cv', label: 'Pulse tenu', ok: a.cv < 22, mid: a.cv < 35, val: a.cv.toFixed(0) + ' %' });",
 "c.push({ k: 'cv', label: 'Pulse held', ok: a.cv < 22, mid: a.cv < 35, val: a.cv.toFixed(0) + ' %' });"),
...
("['Irrégularité du pulse', a.cv.toFixed(0) + ' %', a.cv < 22 ? 'g' : a.cv < 35 ? 'w' : 'b'],",
 "['Pulse unevenness', a.cv.toFixed(0) + ' %', a.cv < 22 ? 'g' : a.cv < 35 ? 'w' : 'b'],"),
...
("+ '<th>Plus long</th><th>Justes</th><th>Pulse</th></tr>'",
 "+ '<th>Longest</th><th>Right</th><th>Pulse</th></tr>'"),
...
("check('un jeu parfaitement régulier a une irrégularité nulle',",
 "check('perfectly steady playing shows zero unevenness',"),
...
("check('la partition affiche des notes', $('score').querySelectorAll('ellipse').length > 0);",
 "check('the score shows notes', $('score').querySelectorAll('ellipse').length > 0);"),
