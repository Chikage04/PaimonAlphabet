...
('<a href="lecture-vue.html">👁️ Lecture à vue</a>', '<a href="lecture-vue.html">👁️ Sight-reading</a>'),
...
("<label class=\"toggle\" title=\"Les conseils sont dits à voix haute, avec les seuils du niveau choisi\"><input type=\"checkbox\" id=\"voiceOn\"> 🔊 Conseils à voix haute</label>",
 "<label class=\"toggle\" title=\"Advice is spoken aloud, using the thresholds of the chosen level\"><input type=\"checkbox\" id=\"voiceOn\"> 🔊 Spoken coaching</label>"),
("if (e.target.checked) { voiceState.last = 0; speak('Conseils à voix haute activés'); }",
 "if (e.target.checked) { voiceState.last = 0; speak('Spoken coaching on'); }"),
("if (rel <= -tolTempo) add('rush', Math.abs(rel) / se, '⏩', 'Tu accélères — reste sur le clic', 'bad', 'Trop vite');",
 "if (rel <= -tolTempo) add('rush', Math.abs(rel) / se, '⏩', 'Rushing — stay with the click', 'bad', 'Too fast');"),
("else if (rel >= tolTempo) add('drag', Math.abs(rel) / se, '⏪', 'Tu ralentis — relance', 'bad', 'Trop lent');",
 "else if (rel >= tolTempo) add('drag', Math.abs(rel) / se, '⏪', 'Dragging — pick it back up', 'bad', 'Too slow');"),
("'〰️', `Irrégulier — ${rSD.toFixed(0)} ms pour ${tolMs.toFixed(0)} autorisés`, 'bad', 'Irrégulier');",
 "'〰️', `Uneven — ${rSD.toFixed(0)} ms against ${tolMs.toFixed(0)} allowed`, 'bad', 'Uneven');"),
(r"if (r < minArt) add('legato-court', (minArt - r) / se, '🔗', 'Notes trop courtes — tiens-les jusqu\'à la suivante', 'warn', 'Trop court');",
 "if (r < minArt) add('legato-court', (minArt - r) / se, '🔗', 'Notes too short — hold each until the next sounds', 'warn', 'Too short');"),
("else if (r > 1.15) add('legato-long', (r - 1.15) / se, '✂️', 'Tu lies trop — relâche un peu plus tôt', 'warn', 'Trop lié');",
 "else if (r > 1.15) add('legato-long', (r - 1.15) / se, '✂️', 'Over-connected — release a little sooner', 'warn', 'Too connected');"),
("'🎚️', `Relâchements irréguliers — ±${rs.toFixed(2)} pour ±${L.artSD.toFixed(2)}`, 'warn', 'Relâchements irréguliers');",
 "'🎚️', `Uneven releases — ±${rs.toFixed(2)} against ±${L.artSD.toFixed(2)}`, 'warn', 'Uneven releases');"),
("`Doigt ${bestF.f} trop ${bestF.d > 0 ? 'fort' : 'faible'}`);",
 "`Finger ${bestF.f} too ${bestF.d > 0 ? 'strong' : 'weak'}`);"),
("`Accent sur la note ${bestP.p + 1}`);", "`Accent on note ${bestP.p + 1}`);"),
("cf.slope > 0 ? 'Tu forces en montant' : 'Tu faiblis en montant');",
 "cf.slope > 0 ? 'Louder going up' : 'Softer going up');"),
("check('un écart de 3,2 passe à Souple', advL === null || advL.key !== 'doigt4',",
 "check('a 3.2 gap passes at Lenient', advL === null || advL.key !== 'doigt4',"),
("advL ? advL.key : 'silence');", "advL ? advL.key : 'silent');"),
("check('… et est repris à Strict', advS && advS.key === 'doigt4',",
 "check('… and is flagged at Strict', advS && advS.key === 'doigt4',"),
("advS ? advS.key + ' : ' + advS.say : 'silence');", "advS ? advS.key + ': ' + advS.say : 'silent');"),
("check('chaque conseil a une version courte à dire',",
 "check('every piece of advice has a short spoken form',"),
("!!(advS && advS.say && advS.say.length <= 40), advS ? '« ' + advS.say + ' »' : 'aucun');",
 "!!(advS && advS.say && advS.say.length <= 40), advS ? '\"' + advS.say + '\"' : 'none');"),
...
("text: HN[best.H] + ' : ' + deCap(best.text),", "text: HN[best.H] + ': ' + deCap(best.text),"),
("text: 'Mains décalées — écoute-les ensemble', cls: 'bad', say: 'Mains décalées'",
 "text: 'Hands out of sync — listen to them together', cls: 'bad', say: 'Hands out of sync'"),
("check('défaut à gauche seulement → la main est nommée',",
 "check('fault on the left only → the hand is named',"),
("a && /gauche/i.test(a.say) && /doigt 4/i.test(a.say), a ? '« ' + a.say + ' »' : 'silence');",
 "a && /left/i.test(a.say) && /finger 4/i.test(a.say), a ? '\"' + a.say + '\"' : 'silent');"),
("check('défaut à droite seulement → la main est nommée',",
 "check('fault on the right only → the hand is named',"),
("a && /droite/i.test(a.say), a ? '« ' + a.say + ' »' : 'silence');",
 "a && /right/i.test(a.say), a ? '\"' + a.say + '\"' : 'silent');"),
("check('défaut des deux côtés → aucune main nommée',",
 "check('fault on both sides → no hand named',"),
("check('une seule main jouée → on ne la nomme pas inutilement',",
 "check('only one hand played → no pointless naming',"),
("a && !/gauche|droite/i.test(a.say), a ? '« ' + a.say + ' »' : 'silence');",
 "a && !/left|right/i.test(a.say), a ? '\"' + a.say + '\"' : 'silent');"),

("check('ondulation de volume → pas de crescendo annoncé',",
 "check('volume wobble → no crescendo announced',"),
("!a || a.key !== 'cresc', a ? a.key + ' : ' + a.say : 'silence');",
 "!a || a.key !== 'cresc', a ? a.key + ': ' + a.say : 'silent');"),
("check('vrai crescendo de 14 par octave → annoncé',",
 "check('real crescendo of 14 per octave → announced',"),
("a && a.key === 'cresc', a ? a.key + ' : ' + a.say : 'silence');",
 "a && a.key === 'cresc', a ? a.key + ': ' + a.say : 'silent');"),
("check('sur 32 notes la portée en hauteur est trop courte pour conclure',",
 "check('over 32 notes the pitch span is too short to conclude',"),
("fitCourt && fitCourt.span < 0.75, fitCourt ? fitCourt.span.toFixed(2) + ' octave' : 'non mesurable');",
 "fitCourt && fitCourt.span < 0.75, fitCourt ? fitCourt.span.toFixed(2) + ' octave' : 'not measurable');"),

# --------------------------------------------------------------- coach -----
("cf.slope > 0 ? 'Tu forces en montant — même volume partout'",
 "cf.slope > 0 ? 'Getting louder as you ascend — keep one level'"),
...
("check('les niveaux se resserrent bien de Souple à Strict', mono, mono ? 'échelle cohérente' : 'relâché sur ' + ou);",
 "check('levels tighten from Lenient to Strict', mono, mono ? 'consistent ladder' : 'loosened on ' + ou);"),
("if (b.even > a.even) pire('rythme', a.even, b.even);",
 "if (b.even > a.even) pire('timing', a.even, b.even);"),
("if (b.finger > a.finger) pire('doigts', a.finger, b.finger);",
 "if (b.finger > a.finger) pire('fingers', a.finger, b.finger);"),
("if (b.grid > a.grid) pire('clic', a.grid, b.grid);",
 "if (b.grid > a.grid) pire('click', a.grid, b.grid);"),
("if (b.minBpm < a.minBpm) pire('tempo mini', a.minBpm, b.minBpm);",
 "if (b.minBpm < a.minBpm) pire('min tempo', a.minBpm, b.minBpm);"),
("if (a.artSD != null && (b.artSD == null || b.artSD > a.artSD)) pire('legato régulier', a.artSD, b.artSD);",
 "if (a.artSD != null && (b.artSD == null || b.artSD > a.artSD)) pire('legato consistency', a.artSD, b.artSD);"),
...
("const HS = { R: 'MD', L: 'MG' };", "const HS = { R: 'RH', L: 'LH' };"),
("const YES_NO = { y: 'oui', n: 'non' };", "const YES_NO = { y: 'yes', n: 'no' };"),
("""done: 'Exercice terminé', wrong: 'Fausses notes', minBpm: 'Tempo minimum',""",
 """done: 'Exercise completed', wrong: 'Wrong notes', minBpm: 'Minimum tempo',"""),
("""even: 'Régularité du rythme', finger: 'Écart de force entre doigts',""",
 """even: 'Timing evenness', finger: 'Force gap across fingers',"""),
("""tempo: 'Tempo tenu', grid: 'Calage sur le clic',""",
 """tempo: 'Tempo held', grid: 'Lock to the click',"""),
("""artSD: 'Régularité du legato', art: 'Notes liées'""",
 """artSD: 'Legato consistency', art: 'Notes connected'"""),
("setRtState('✗ ' + chkLabel(f) + ' : ' + chkVal(f) + (f.lim ? ' (il faut ' + f.lim + ')' : '')",
 "setRtState('✗ ' + chkLabel(f) + ': ' + chkVal(f) + (f.lim ? ' (needs ' + f.lim + ')' : '')"),
("+ `<span class=\"res\">${r.clean ? '✓ propre' : '✗ ' + chkText(r.fail0)}</span></li>`",
 "+ `<span class=\"res\">${r.clean ? '✓ clean' : '✗ ' + chkText(r.fail0)}</span></li>`"),
(": r.fail0 ? '✗ ' + chkText(r.fail0)", ": r.fail0 ? '✗ ' + chkText(r.fail0)"),
("? `Dernière séance : ${d.date} (${(DAILY_LEVELS[d.level] || {}).label || ''}) — ${d.validated} validé${d.validated > 1 ? 's' : ''}, arrêté au n°${d.reached}`",
 "? `Last session: ${d.date} (${(DAILY_LEVELS[d.level] || {}).label || ''}) — ${d.validated} passed, stopped at no. ${d.reached}`"),
# ------------------------------------------------------- config séance -----
...
("['irrégularité', num(h.res, 1, ' ms')], ['CV', num(h.cv, 1, ' %')],",
 "['unevenness', num(h.res, 1, ' ms')], ['CV', num(h.cv, 1, '%')],"),
("['écart doigts', num(h.fingerSD, 1)], ['force', num(h.vel, 1)],",
 "['finger gap', num(h.fingerSD, 1)], ['velocity', num(h.vel, 1)],"),
...
("'Strict à ♩=' + bpmHi + ' : ' + tolFast.toFixed(0) + ' ms, Souple à ♩='",
 "'Strict at ♩=' + bpmHi + ': ' + tolFast.toFixed(0) + ' ms, Lenient at ♩='"),
("+ bpmLo + ' : ' + tolSlow.toFixed(0) + ' ms');",
 "+ bpmLo + ': ' + tolSlow.toFixed(0) + ' ms');"),
